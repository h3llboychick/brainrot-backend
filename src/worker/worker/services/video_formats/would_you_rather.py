...
class WouldYouRatherFormat(VideoFormatStrategy):
    ...
    def generate(
        ...
        event_publisher.publish_event(
            job_id, "generating_script", stage="generating_script"
        )
        ...
        event_publisher.publish_event(
            job_id, "script_generated", stage="generating_script"
        )
        logger.info(f"Script generated with {len(script)} scenarios")

        # Step 2: Fetch assets
        event_publisher.publish_event(
            job_id, "fetching_assets", stage="fetching_assets"
        )
        ...
        video_url = video_storage_manager.upload_video(
            object_name=f"{job_id}.mp4", file_path=str(output_video_path)
        )

        event_publisher.publish_event(
            job_id, "uploaded", stage="uploading", video_url=video_url
        )

        return output_video_path

    def _generate_script(self, ai_client, prompt: str) -> Dict[str, Any] | None:
        """Generate script using AI with retry logic."""

        def _attempt_generation():
            messages = build_chat_messages(
                system_prompt=WOULD_YOU_RATHER_PROMPT, user_prompt=prompt
            )
    ...
    def _fetch_assets(
        ...
        def generate_and_save_voiceover(
            output_file: str,
            text: str,
            voice_settings: VoiceSettings = VoiceSettings(
                stability=0.1, similarity_boost=0.8
            ),
        ) -> None:
        ...
        for scenario_key, scenario in script.items():
            for option_key, option in scenario.items():
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to generate voiceover: {audio_filename}"
                    )
    ...
    def _assemble_video(
        ...
        timer_and_ringing_sound_duration = get_mp3_length(
            str(timer_and_ringing_audio_path)
        )
        ...
        for key in script.keys():
            ...
            scenario_option_a_audio_duration = get_mp3_length(
                str(audios_dir / f"{scenario_option_a}.mp3")
            )
            scenario_option_b_audio_duration = get_mp3_length(
                str(audios_dir / f"{scenario_option_b}.mp3")
            )

            first_option_percents = randint(1, 100)  # nosec: B311
        ...
        inputs[str(background_video_path)] = (
            f"-ss 00:00:00 -t {current_video_duration} "
        )
