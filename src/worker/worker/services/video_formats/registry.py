...
class VideoFormatRegistry:
    ...
    @classmethod
    def register(
        cls, format_class: Type[VideoFormatStrategy]
    ) -> Type[VideoFormatStrategy]:
        """
    ...
    def register_format(cls, format_class: Type[VideoFormatStrategy]) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to instantiate format class {format_class.__name__}: {e}"
            )
            raise ValueError(
                f"Format class {format_class.__name__} must be instantiable: {e}"
            )
        ...
        logger.info(
            f"Registered video format: '{format_name}' ({format_class.__name__})"
        )
