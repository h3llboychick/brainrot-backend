...
class PexelsClient:
    ...
    def get_photo(self, keyword: str, photo_path: str) -> None:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"Received response from Pexels. Number of results: {data['total_results']}"
            )
            if data["total_results"] > 0:
                ...
                with open(photo_path, "wb") as f:
                    f.write(requests.get(image_url).content)  # nosec: B113
            else:
                logger.warning(f"No results found for keyword: {keyword}")
        else:
            logger.error(
                f"Failed to fetch photo from Pexels. Status code: {response.status_code}, Response: {response.text}"
            )
