...
class YouTubePublisher(PlatformPublisher):
    ...
    def from_credentials_dict(cls, credentials_dict: dict) -> 'YouTubePublisher':
        scopes = credentials_dict.get("scopes") or ["https://www.googleapis.com/auth/youtube.upload"]
        ...
        info = {
            ...
            "client_id": credentials_dict.get("client_id") or worker_settings.GOOGLE_CLIENT_ID,
            "client_secret": credentials_dict.get("client_secret") or worker_settings.GOOGLE_CLIENT_SECRET,
            "scopes": scopes,
        }

        missing = [key for key in ("refresh_token", "token_uri", "client_id", "client_secret") if not info.get(key)]
        if missing:
        ...
        if not credentials.valid:
            if credentials.refresh_token:
                ...
                logger.error("YouTube credentials lack a refresh token; user must re-authorize account.")
                raise RuntimeError("YouTube credentials lack refresh token; user must re-authorize.")
