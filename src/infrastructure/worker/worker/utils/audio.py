...
def change_speed(input_file: str, output_file: str, speed_coefficient: float) -> None:
    logger.info(f"Changing speed of {input_file} by a factor of {speed_coefficient}, saving to {output_file}")
    ...
    if response_upload.status_code == 200:
        ...
        if response_speed.status_code == 200:
            logger.info(f"Speed change request successful for {file_path}. Trying to download the modified file...")
            ...
            if response.status_code == 200:
                logger.info(f"Speed changed successfully, downloading the file to {output_file}")
                ...
                logger.error(f"Failed to download the file. Status code: {response.status_code}, Response: {response.text}")
        else:
...
def get_mp3_length(file_path: str) -> float | None:
    try:
