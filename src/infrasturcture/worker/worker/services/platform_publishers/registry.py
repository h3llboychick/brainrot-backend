...
class PlatformPublisherRegistry:
    ...
    @classmethod
    def register(cls, publisher_class: Type[PlatformPublisher]) -> Type[PlatformPublisher]:
        """
        ...
        if platform_name in cls._publishers:
            logger.warning(f"Platform publisher '{platform_name}' already registered, overwriting")
