...
class CredentialsManager:
    def __init__(self, encryption_manager: EncryptionManager, social_accounts_repository: SocialAccountsRepository):
    ...
    def get_decrypted_credentials(self, user_id: str, platform: str) -> dict:
        social_account = self.social_accounts_repository.get_social_account(user_id, platform=platform)
        if not social_account:
            raise ValueError(f"No social account found for user_id={user_id} platform={platform}")
