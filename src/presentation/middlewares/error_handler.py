...
def setup_exception_handlers(app: FastAPI):
    ...
    async def user_not_verified_exception_handler(
        request, exc: UserNotVerifiedError
    ):
        return JSONResponse(
            status_code=status.HTTP_403_FORBIDDEN, content={"message": str(exc)}
        )
    ...
    async def oauth_credentials_error_handler(
        request, exc: OAuthCredentialsError
    ):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def oauth_token_exchange_error_handler(
        request, exc: OAuthTokenExchangeError
    ):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"message": str(exc), "platform": exc.platform},
    ...
    async def oauth_authorization_error_handler(
        request, exc: OAuthAuthorizationError
    ):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"message": str(exc), "platform": exc.platform},
