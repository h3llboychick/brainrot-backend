...
def setup_exception_handlers(app: FastAPI):
    ...
    async def invalid_token_payload_exception_handler(request, exc: InvalidTokenPayloadError):
        return JSONResponse(
            status_code=status.HTTP_401_UNAUTHORIZED,
            content={"message": str(exc)}
        )
    ...
    async def invalid_credentials_exception_handler(request, exc: InvalidCredentialsError):
        return JSONResponse(
            status_code=status.HTTP_401_UNAUTHORIZED,
            content={"message": str(exc)}
        )
    ...
    async def verification_code_not_found_exception_handler(request, exc: VerificationCodeNotFoundError):
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content={"message": str(exc)}
        )
    @app.exception_handler(InvalidVerificationCodeError)
    async def invalid_verification_code_exception_handler(request, exc: InvalidVerificationCodeError):
        return JSONResponse(
    ...
    async def expired_social_account_credentials_handler(request, exc: ExpiredSocialAccountCredentialsError):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"message": str(exc)}
        )
    @app.exception_handler(InvalidSocialAccountCredentialsError)
    async def invalid_social_account_credentials_handler(request, exc: InvalidSocialAccountCredentialsError):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"message": str(exc)}
        )
    ...
    async def not_found_social_account_handler(request, exc: NotFoundSocialAccountError):
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content={"message": str(exc)}
        )