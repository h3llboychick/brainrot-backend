...
from src.domain.dtos.auth.tokens import RefreshAccessTokenDTO
...
from src.domain.exceptions.oauth import OAuthAuthorizationError

from src.presentation.schemas import (
    UserEmailRegistrationRequest, UserEmailVerificationCodeRequest,
    UserEmailLoginRequest, 
    AuthenticationResponse, RefreshAccessTokenResponse, 
...
async def register_router(
    ...
    use_case: Annotated[RegisterUserEmailUseCase, Depends(get_register_user_email_use_case)],
):
...
async def verify_email_router(
    ...
    use_case: Annotated[VerifyUserEmailUseCase, Depends(get_confirm_user_email_use_case)]
):
    await use_case.execute(
...
async def google_callback(
    request: Request, 
    use_case: Annotated[SignInWithGoogleUseCase, Depends(get_signin_with_google_use_case)]
):
    """
...
async def refresh_token_router(
    ...
    use_case: Annotated[RefreshAccessTokenUseCase, Depends(get_refresh_access_token_use_case)],
):
    result = await use_case.execute(
